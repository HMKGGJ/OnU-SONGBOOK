...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 부분 갱신: 해당 song-card만 즐겨찾기 버튼 갱신
  function updateSongCardFavoriteUI(songId, isFav) {
    const card = refs['song-grid'].querySelector(`.song-card[data-id="${songId}"]`);
    if (!card) return;
  ...
  async function render() {
    ...
    if (viewMode !== 'songs') {
      ...
      const items = viewMode === 'categories' ? [...new Set(songs.flatMap(s => s.categories || []))] : [...new Set(songs.map(s => s.artist))];
      items.forEach(tag => {
        const div = document.createElement('div');
        const song = songs.find(s => viewMode === 'categories' ? (s.categories || []).includes(tag) : s.artist === tag);
  ...
  supabase.auth.onAuthStateChange(async () => {
    ...
    // [수정] 로그인/아웃 시 유저 정보 및 즐겨찾기 갱신 후 렌더링
    await updateCurrentUser();
    await updateCurrentFavorites();
    render(); 
  });
